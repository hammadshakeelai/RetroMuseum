...
- The page passes the block to v86 with the disk's `url` prefixed by `<base>images/`, adding only `wasm_path`, `bios`, `vga_bios`, `screen: { container, use_graphical_text: false }`, and `autostart: true`.
- The screen container has exactly the structure from v86's `examples/basic.html`: a `<div style="white-space: pre; font: 14px monospace; line-height: 14px">` followed by a `<canvas style="display: none">`. The container itself has `width: max-content`, so it is as wide as what v86 draws.
...
- The page never calls `screen_set_scale`. v86 sizes its own layers: it divides the canvas size by a fractional `devicePixelRatio` and doubles canvases up to 640 pixels wide on large windows. So `fitScreen(screen, area)` in `src/emulator/screen.ts` clears its earlier transform, measures whichever layer is showing (the canvas unless its `display` is `none`, otherwise the text layer), and sets `transform: scale(s)` on that layer only. It runs on resize, on v86's `screen-set-size`, and on entering or leaving full screen. Found in Task 7: graphical text mode left ghosted rows in FreeDOS and ELKS, and on a 1920×1080 window v86 doubled a 320×200 canvas to twice its area.
- **Full screen** calls `requestFullscreen()` on the screen container, then captures the mouse. v86's `screen_go_fullscreen` only looks for an element with the id `screen_container`, which RetroMuseum's pages don't have.
...
- Produces:
  ...
  - Collection `exhibits`; `CollectionEntry<"exhibits">` `data`: `title`, `maker`, `year`, `family`, `license`, `summary`, `downloadEstimateMB`, `screenshotWaitSeconds`, `screenshotInput?`, `facts: Record<string, string>`, `tryThis: string[]`, `homepage?`, `screenshot` (image metadata), `sources: string[]`, `diskImage?: DiskImage`, `v86?: V86Block`, `copyShProfile?: string`.
